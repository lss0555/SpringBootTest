...
; (function ($) {
    $.fn.easyUpload = function (opts) {
        var uploadFileTempResult=new Array()
        var inituploadedFileNumTempResult=new Array()
        var uploadFilesResult=new Array()
        var uploadFilesNameResult=new Array()
        var defaults = {
            allowFileTypes: '*.jpg;*.pdf',//允许上传文件类型，格式'*.pdf;*.doc;'  *.pdf;*.doc;*.docx;*.jpg;*.png;*.rar;*.ppt
        ...
        this.each(function (index, element) {
            ...
            var easyManager = {
                init: function () {
                    ...
                    $html += option.multi ? '<span style="display: none" class="easy_upload_head_btn1 noselect">上传</span>' : '';
                ...
                bindHead: function () {
                    ...
                    $('.fileInput').off('change').change(function () {
                        ...
                        _this._checkFile(fileArr, this,1);
                    ...
                    $('.easy_upload_head_btn1').off('click').click(function(){
                        var fileList=opts.initfileArrList;
                        if(fileList!=null && fileList!=""){
                            for (var i=0;i<fileList.length;i++){
                                var fileNames= fileList[i].name.split(",");
                                uploadFilesResult.push(fileNames[0]);
                                uploadFilesNameResult.push(fileNames[1]);
                                inituploadedFileNumTempResult.push(i)
                            }
                            option.upLoadFiles(uploadFilesResult,uploadFilesNameResult);
                            console.log("初始化文件:"+fileList);
                            var count = 0;
                            var fileArr = [];
                            var files = fileList;
                            for (var i = 0; i < files.length; i++) {
                            ...
                            $('.fileInput').val('').attr('data-count', count);
                            _this._checkFile(fileArr, this,0);
                            $(this).parent().find('.head_check').html('&#xe693;').attr('data-checked', 'no');
                        }
                    });
                ...
                //文件约束
                _checkFile: function (fileArr, target,type) {
                    ...
                    // }
                    if(type==0){
                        this._renderFile1(fileArr, target,type);
                    }else {
                        this._renderFile(fileArr, target,type);
                    }
                },
                _renderFile: function (fileArr, target,type) {
                    ...
                    function render(file) {
                        ...
                        if (fileType == 'bmp' || fileType == 'jpg' || fileType == 'jpeg' || fileType == 'png' || fileType == 'gif') {
                            ...
                            preview = '<img style="margin:15px auto;width: 60px; height: 50px;"  class="easy_upload_img" src="' + imgSrc + '" />';
                        }
                        else if (fileType == 'rar' || fileType == 'zip' || fileType == 'arj' || fileType == 'z') {
                            preview = '<i style="margin: 0 20px;" class="easy_upload_icon easyUploadIcon">&#xe69d;</i>';
                        } else {
                            preview = '<i style="margin: 0 20px;" class="easy_upload_icon easyUploadIcon">&#xe64d;</i>';
                        }
                        var sHtml = '';
                        ...
                        var $html = '';
                        $html += '<li onclick="javascript:window.location.href=\'https://wx.qzzg.cn/javatest/lss_sso_server/uploadFile/pdf%E6%96%87%E4%BB%B6%E6%B5%8B%E8%AF%95.pdf\'" class="easy_upload_queue_item" data-index="' + file.index +'">';
                        $html += option.showPreview ? '<div style="width: 200px; height: 90px;"   class="easy_upload_preview queue_item-section">' + preview + '</div>' : '';
                        ...
                        $html += '<p style="width: 100px;" class="easy_upload_percent">0%</p>';
                        $html += '</div>';
                        ...
                        $html += '</div>';
                        // $html += '<div style="display: none" class="easy_upload_checkone queue_item-section">';
                ...
                //=====================================================初始化文件
                _renderFile1: function (fileArr, target,type) {
                    var queueUl = $(target).parent().parent().find('.easy_upload_queue');
                    //遍历包装
                    for (var i = 0; i < fileArr.length; i++) {
                        if (option.multi) {
                    ...
                    function render1(file) {
                        var preview;
                        var f = file.file;
                        var fileNames=f.name.split(",");
                        var fileType = fileNames[0].split('.').pop();
                        if (fileType == 'bmp' || fileType == 'jpg' || fileType == 'jpeg' || fileType == 'png' || fileType == 'gif') {
                            var imgSrc = URL.createObjectURL(f);
                            preview = '<img style="margin:15px auto;width: 60px; height: 50px;"  class="easy_upload_img" src="' +basePath+ "/noticeFile/"+fileNames[1] + '" />';
                        }
                        else if (fileType == 'rar' || fileType == 'zip' || fileType == 'arj' || fileType == 'z') {
                            preview = '<i style="margin: 0 20px;" class="easy_upload_icon easyUploadIcon">&#xe69d;</i>';
                        } else {
                        ...
                        var sHtml = '';
                        sHtml += '<p style="color: #89de4d;" class="status ">已上传</p>';
                        var $html = '';
                        $html += '<li class="easy_upload_queue_item" data-index="' + file.index +'">';
                        $html += option.showPreview ? '<div style="width: 200px; height: 90px;"   class="easy_upload_preview queue_item-section">' + preview + '</div>' : '';
                        $html += '<div class="easy_upload_file1 queue_item-section">';
                        $html += '<p class="easy_upload_filename">'+ fileNames[0] +'</p>';
                        $html += '<p class="easy_upload_progress">';
                        $html += '<span style="width: 100%" class="easy_upload_bar"></span>';
                        $html += '</p>';
                        $html += '</div>';
                        $html += '<div class="easy_upload_file2 queue_item-section">';
                        // $html += '<p class="easy_upload_fiesize">' + F.formatFileSize(f.size) +'</p>';
                        $html += '<p style="width: 100px;" class="easy_upload_percent">100%</p>';
                        $html += '</div>';
                        $html += '<div class="easy_upload_status queue_item-section">';
                        $html += file.allow ? sHtml : '<p class="status status6">文件不允许</p>';
                        $html += '</div>';
                            ...
                            $.ajax({
                                ...
                                success: function (res) {
                                    ...
                                    if (res.errCode==0){
                                        ...
                                        uploadFileTempResult.push(file.index+","+res.model);
                                        uploadFilesResult.push(res.stringData);
                                        uploadFilesNameResult.push(res.model);
                                        option.upLoadFiles(uploadFilesResult,uploadFilesNameResult);
                                        if (option.multi) {
                ...
                _deleFiles: function(arr,target) {
                    ...
                    arr.forEach(function(item){
                        ...
                        if (option.multi)
                            ...
                            var fileList=opts.initfileArrList;
                            var filename; var fileOriname; var isEdit=0;
                            if(fileList!=null && fileList!=""){
                                var nums=inituploadedFileNumTempResult.length;
                                if(item>(nums-1)){
                                    //不是编辑文件
                                    filename=file.file.name;
                                    for(var i=0;i<uploadFileTempResult.length;i++){
                                        var file=uploadFileTempResult[i].split(",");
                                        if(item==file[0]){
                                            fileOriname=file[1];
                                            break;
                                        }
                                    ...
                                    fileOriname=file.file.name.split(",")[1];
                                    isEdit=1;
                                }
                            }else {
                                filename=file.file.name;
                            ...
                            //判断是否已编辑删除文件
                            if(uploadFilesResult.length>0){
                                for (var i=0;i<uploadFilesNameResult.length;i++){
                                    if(uploadFilesNameResult[i]==fileOriname){
                                        //删除临时文件
                                        $.ajax({
                                            url: basePath+"/admin/oaNotice/delFile",
                                            type: "POST",
                                            data: JSON.stringify({
                                                i_file_name: fileOriname,
                                                i_isedit: isEdit
                                            }),
                                            dataType: 'json',
                                            ...
                                            success: function (res) {
                                                ...
                                                if (res.errCode==0){
                                                    ...
                                                    uploadFilesResult.splice(i,1);
                                                    uploadFilesNameResult.splice(i,1);
                                                    uploadFileTempResult.splice(item,1);
                                                    option.upLoadFiles(uploadFilesResult,uploadFilesNameResult);
                                                }else {
